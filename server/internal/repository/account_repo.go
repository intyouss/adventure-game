...
func (r *AccountRepo) FindByPhone(ctx context.Context, phone string) (*model.Account, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`SELECT id, phone, email, password_hash, created_at FROM accounts WHERE phone = $1`, phone,
...
func (r *AccountRepo) FindByEmail(ctx context.Context, email string) (*model.Account, error) {
	a := &model.Account{}
	err := r.db.QueryRowContext(ctx,
		`SELECT id, phone, email, password_hash, created_at FROM accounts WHERE email = $1`, email,
	).Scan(&a.ID, &a.Phone, &a.Email, &a.PasswordHash, &a.CreatedAt)
	if err == sql.ErrNoRows {
		return nil, nil
	}
...
func (r *AccountRepo) FindByID(ctx context.Context, id int64) (*model.Account, error) {
	a := &model.Account{}
	err := r.db.QueryRowContext(ctx,
		`SELECT id, phone, email, password_hash, created_at FROM accounts WHERE id = $1`, id,
	).Scan(&a.ID, &a.Phone, &a.Email, &a.PasswordHash, &a.CreatedAt)
	if err == sql.ErrNoRows {
		return nil, nil
	}
